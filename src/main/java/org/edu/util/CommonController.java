...
public class CommonController {
	 ...
	 * 사용용도: 게시물상세보기 첨부파일이 이미지면 미리보기 이미지가보이도록, 이미지가 아니면 다운로드 링크만 보이도록
	 * 사용용도2: 메인페이지 최근 겔러리 게시물에서 첨부파일이 있으면 미리보기 이미지가 나오게 되는데 
	 * 만약 첨부파일이 이미지가 아니면 대체이미지를 보이기 위해서 확장자 체크가 필요합니다.
	...
	//첨부파일 업로드할 경로를 변수값으로 가져옴 servlet-context.xml에 있는 내용 (@Resource쓰면) 서블렛에서 가져올수있다.
	@Resource(name="uploadPath")
	private String uploadPath;//위 uploadPaht영역의 값을 uploadPath 멤버(전역)변수에 저장.
	
	public String getUploadPath() {	//컨트롤러에서 파일 삭제시 호출 된다.
		return uploadPath;
	}
	...
	@ResponseBody // 이 애노테이션으로 지정된 메서드는 페이지 이동처리가 아니고, RESTAPI처럼 구현 결과 내용을 현재페이지에서 전송 받습니다.
	public FileSystemResource download(
			...
			) throws Exception {	//파일 시스템리소스로 현재 페이지에서 반환 받음	
		File file = new File(uploadPath + "/" + save_file_name); //다운받을 경로 지정
		response.setContentType("application/download; utf-8");//파일 내용중 한글이 깨지는거 방지
	...
	public String fileUpload(MultipartFile file) throws IOException {
		...
		return saveFileName;//copy로 업로드 이후에 저장된 real_file_name 스트링 문자열값 1개를 반환한다.		
	}
	...
	@ResponseBody//메서드 응답을 내용만 반환받겠다고 명시 RestAPI

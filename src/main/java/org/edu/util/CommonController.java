...
public class CommonController {
	...
	//REST-API서비스로 사용할때 컨트롤러로 사용(아래) @ResponseBody애노테이션은 json텍스트데이터를 반환함
	//아래는 RestAPI백엔드단, Ajax(jsp) Rest-API의 프론트엔드단.
	@RequestMapping(value="/id_check",method=RequestMethod.GET)
	@ResponseBody
	public String id_check(@RequestParam("user_id") String user_id) {
		String result ="0";//아이디 중복값을 체크하는 변수 초기값은 중복값없음
		//Rest-API서비스에서는 스프링을 통해서 Ajax로 에러메시지를 받을 수 없기 때문에 여기서 에러를 처리해야 합니다.
		try {

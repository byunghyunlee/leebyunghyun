 ...
 * http://localhost:8080/ 요청을 받았을때, 변수를 만들어서 화면에 뿌려준다.
 */
@Controller
public class HomeController {
	 ...
	 * homeController.java 를 저장하면, 이클립스에서는 HomeContoller.class 실행가능한 파일을 생성
	...
	public String index() { //메서드(함수)구성: 리턴(출력)형태지정 메서드명(매개변수) {구현내용} String index(){}
		//상단 리퀘스트매핑에서 지정한 경로로 접근을 하게 되면, 아랭 있는 index()메서드를 실행해라.
		//메서드명 제일 앞에있는 public는 접근권한에 대한 키워드인데, public(공공-다른클래스에서 접근가능),
		//private(내부-현재 HomeController클래스에서만 접근가능)
		// public String index() {} 자바(스프링) 매서드 기본형태
		// $(document).ready(function(){}); j쿼리 기본형태
		//리턴반환값으로 사용한 String 클래스는 문자열을 반환하고, 
		//여기서는 index.jsp와 매칭되는 문자열을 반환.
...
/*	@RequestMapping(value = "/", method = RequestMethod.GET)
	public String home(Locale locale, Model model) {
		...
		System.out.println("함수와 -C언어나 자바스크립트-와 매서드-자바,스프링에서는 사용함-는 같은 대상을 가리킵니다.");
		System.out.println("함수-메서드는 함수명('매개변수-입력값'){구현내용}형식이고, 입력값->출력값 구현됩니다.");
		Date date = new Date();//Date 날짜 관련 클래스형 변수 date 선언했습니다. date변수 메서드를 사용 가능하게 됨
		//data 변수는 실행가능한 변수=클래스형변수=object오브젝트=인스턴스 라고 합니다.(위)
		DateFormat dateFormat = DateFormat.getDateTimeInstance(DateFormat.LONG, DateFormat.LONG, locale);
		//DateFormat 클래스형 변수 date이 선언되었고, 실행가능한 변수로 되었습니다.(위)
		...
		//위 model이라는 클래스형 변수를 이용해서, serverTime변수값을 아래 home(생략.jsp)로 전송해줌.

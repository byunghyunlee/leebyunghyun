 ...
 * 현재 제이유닛4클래스를 사용.
 * 단위테스트는 톰캣이 실행되지 않아도 작동이 되야 한다. 
 * 그런데 안되고 있으니 그래서, 테스트 클래스 상단에 servlet-context. xml 이러한 설정파일을 불러들여야 실행이 가능하다.
 * @author 이병현
 *
 */
...
public class DataSourceTest {
	...
	public void dbConnectionTest()  {
		try {//내부에서 {} 에러발생시 실행을 중지하고, cath{}구문이 실행됩니다.
			Connection connection = dataSource.getConnection();//DB커넥션 오브젝트 만드는 과정.
			System.out.println("데이터베이스 접속이 성공하였습니다.");
		} catch (SQLException e) {     
			System.out.println("데이터베이스 접속에 실패하였습니니다. 왜냐하면 " + e);
			e.printStackTrace();
		}//DB커넥션 오브젝트 만드는 과정. new 사용안함.
		//try~catch구문은 테스트에서만 사용하고, 스프링에서는 throw Exception으로 예외를 스프링으로 보냅니다.
	}
	@Test
	public void test() {
